...
async function loadProductos() {
    ...
    // Migración desde clave antigua si existe
    ...
    if (rawOld) {
        try {
            ...
            // eliminar la clave antigua para evitar confusiones
...
function formatCurrency(v) {
    // use non-breaking space after the colón symbol to prevent wrapping
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    cancelBtn.addEventListener('click', () => { form.reset(); editingSku = null; closeModal(); });

    form.addEventListener('submit', (e) => {
        e.preventDefault();
        const sku = skuInput.value.trim();
        const nombre = nombreInput.value.trim();
        if (!sku || !nombre) { formMsg.textContent = 'SKU y Nombre son obligatorios'; return; }
        const nuevo = {
            sku,
            nombre,
        ...
        if (editingSku) {
            ...
            if (findIndexBySku(productos, sku) !== -1) { formMsg.textContent = 'El SKU ya existe'; return; }
    ...
    document.querySelector('#productosTable tbody').addEventListener('click', (e) => {
